...
export const onRequestPatch = async ({ request, env }: Context) => {
  ...
  await env.DB.prepare("UPDATE users SET full_name = ?, phone = ?, region = ?, commune = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?").bind(fullName, cleanText(payload.phone, 30) || null, cleanText(payload.region, 80) || null, cleanText(payload.commune, 80) || null, session.id).run();
