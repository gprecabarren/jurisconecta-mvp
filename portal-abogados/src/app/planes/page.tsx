...
export default function PlansPage() {
  ...
  useEffect(() => { void fetch("/api/lawyer/plans").then((response) => response.ok ? response.json() : Promise.reject()).then((result: { plans: Plan[]; profile: Profile | null }) => { setPlans(result.plans); setProfile(result.profile); }).catch(() => undefined); }, []);
